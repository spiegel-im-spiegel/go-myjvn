...
func TestSortReverse(t *testing.T) {
	...
	if string(json) != res3b {
		t.Errorf("Unmarshal().JSON() = \"%v\", want \"%v\".", string(json), res3b)
	}
}
...
func TestSort(t *testing.T) {
	...
	vuln1.Append(vuln2)
	vuln1.SortByID(false)
	json, err := vuln1.JSON("  ")
	if err != nil {
		t.Errorf("JSON() = \"%v\", want nil.", err)
		return
	}
	if string(json) != res3c {
		t.Errorf("Unmarshal().JSON() = \"%v\", want \"%v\".", string(json), res3c)
	}
}
